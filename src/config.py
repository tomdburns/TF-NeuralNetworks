...
#        if '.csv' in self.job_name:
#            self.job_name = self.job_name.split('.csv')[0]
#            print self.job_name
    ...
    def _init_logging(self):
        ...
        if '.csv' in self.job_name:
            logfile_name = filename=self.job_name.split('.csv')[0]
        else:
            logfile_name = filename=self.job_name

        # Easier to do simple file configuration then add the stdout
        logging.basicConfig(level=file_level,
                            format='[%(asctime)s] %(levelname)s %(message)s',
                            datefmt='%Y%m%d %H:%M:%S',
                            filename=logfile_name + '.flog',
                            filemode='a')
    ...
    def load_job_defaults(self):
        ...
        job_ini_path = os.path.join(self.job_dir, self.job_name + '.ann')
        try:
    ...
    def load_job_type(self, job_type):
        ...
        job_type_ini_path = os.path.join(home_dir, '.anns', job_type + '.ann')
        try:
